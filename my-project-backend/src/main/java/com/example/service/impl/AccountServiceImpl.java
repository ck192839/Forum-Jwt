...
public class AccountServiceImpl extends ServiceImpl<AccountMapper, Account> implements AccountService {
   ...
   @Resource
    ...
    public String registerEmailAccount(EmailRegisterVO emailRegisterVo) {//注册账号,todo:添加验证码类型
        ...
        Account account = new Account(null,username,password,email,Const.ROLE_DEFAULT,null,new Date(),false, false);
        if (!this.save(account)) return "注册失败！请联系管理员";
        else{
    ...
    @Override
        ...
        public String resetConfirm(ConfirmResetVO confirmResetVo) {//重置密码_1.确认验证码
            ...
            if(code==null)
                return "请先获取验证码";
            if(!confirmResetVo.getCode().equals(code)) return "验证码错误";
            return null;
    }
    @Override
