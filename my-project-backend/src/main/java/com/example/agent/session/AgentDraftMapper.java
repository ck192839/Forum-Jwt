...
public interface AgentDraftMapper extends BaseMapper<AgentDraft> {
    @Select("""
            SELECT id, session_id, version, editor_version, title, topic_type_id,
    ...
    @Insert("""
            INSERT INTO agent_draft (
                session_id, version, editor_version, title, topic_type_id,
                body_markdown, citations_json, created_at, updated_at
            ) VALUES (
                #{sessionId}, #{version}, #{editorVersion}, #{title}, #{topicTypeId},
